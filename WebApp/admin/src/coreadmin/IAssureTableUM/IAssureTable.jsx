		...
		if (searchText && searchText.length !== 0) {
			...
			axios.post('/api/users/get/searchlist/delete', data)
				.then((res) => {
					...
					// var listdata = {
					// 	"startRange"        : parseInt(data.startRange),
					// 	"limitRange"        : parseInt(data.limitRange), 
					//   }  
					// axios.post('/api/users/get/list', listdata)
					// 	.then((res) => {
					...
					// 				deletedOn: a.statusupdatedAt !== "-" ? moment(a.statusupdatedAt).format("LLLL") : "-",
																...
																<td className="textAlignCenter">
																	{
																		...
																		{/*this.props.editId && this.props.editId === value._id ? null : <i className={"fa fa-file " + value._id} id={value._id} data-toggle="modal" title="Delete" data-target={`#${value._id}-userDetails`} onClick={this.showUserDetails.bind(this)}></i>*/}
																				{/*&nbsp; &nbsp;*/}
																	...
																	}
																	{
																		this.props.DeletedUsersTable ?
																			<div>
																				{this.props.editId && this.props.editId === value._id ? null : <button className="btn deleteBtn " id={value._id + '-Delete'} data-toggle="modal" title="Delete" data-target={`#${value._id}-rm`} >Delete Permanently</button>}{"\n"}
																				{this.props.editId && this.props.editId === value._id ? null : <button className="btn deleteBtn " id={value._id} onClick={this.restoreUser.bind(this)}>Restore User</button>}
																			</div>
																			: null
																	}
