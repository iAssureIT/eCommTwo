...
class BasicInfo extends Component {
  ...
  supplier(event) {
    event.preventDefault();
    if ($('#BasicInfo').valid()) {
      console.log("inside add vendor",true);
      var userDetails = {
        "firstName": this.state.companyName,
        "lastName": this.state.companyName,
        "emailId": this.state.emailId,
        "pwd": "anashandicraft123",
        "mobileNumber": this.state.mobileNumber,
        "status": "Active",
        "roles": "vendor"
      }
      console.log("userdetails:",userDetails);
      console.log("venderId:",this.state.vendor_ID);
      console.log("venderId prps:",this.props.match.params.vendor_ID);

      if(this.props.match.params.vendor_ID){
        console.log("vendor id",this.props.match.params.vendor_ID);
        if(this.state.vendorRadio === 'new'){
          ...
          console.log("New vendor formValues:", formValues);
          axios.patch('/api/vendors/patch', formValues)
          .then((response) => {
            console.log('response', response.data);
            swal(response.data.message);
        ...
        if(this.state.vendorRadio === 'new'){
          ...
          .then((res) => {
            ...
            console.log("formValues after inserting vendor details into user:",formValues);
