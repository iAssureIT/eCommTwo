...
export default class DispatchedOrdersList extends Component{
  ...
  getOrders(){
    ...
    .then((resdata)=>{
      ...
      axios.get("/api/orders/get/orderlist/Dispatch/"+resdata.data._id)
            .then((response)=>{
              var UsersArray = [];
  ...
  render(){
    return(
      <div>
      <AdminOrdersList tableTitle={'Dispatched Order List'} data={this.state.data} allProductsArray={this.state.allProductsArray} showStatusFilter="false" getOrdersFun={this.getOrders}/>
      </div>
      );
