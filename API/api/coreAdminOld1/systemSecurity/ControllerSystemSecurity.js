...
exports.user_signup_user_email_otp = (req,res,next)=>{
	console.log('Inside API req.body: ', req.body);
	if(req.body.email && req.body.pwd && req.body.role){
		var emailId = req.body.email;
...
exports.check_userID_EmailOTP = (req,res,next)=>{
	console.log("emailOtp:" ,req.params.emailotp);
				...
				bcrypt.hash(req.body.pwd,10,(err,hash)=>{
				    ...
				    .then(data=>{
				        if(data.nModified == 1){
				            ...
				            res.status(401).json("PASSWORD NOT UPDATED");
				        }
				    })
				    .catch(err =>{
				        console.log(err);
...
exports.add_user_address = (req, res, next) => {
	...
	console.log("hi");
	console.log("inside add user address",req.body);
	User.updateOne(
...
exports.add_delivery_address = (req, res, next) => {
	// console.log("inside delivery req body:",req.body.user_ID);
	User.updateOne(
