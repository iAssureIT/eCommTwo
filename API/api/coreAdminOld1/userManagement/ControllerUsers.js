...
exports.user_signup_user_email_otp = (req,res,next)=>{
	if(req.body.role && req.body.email && req.body.pwd){
		console.log("Inside usersignup send otp");
		User.find({emails:{$elemMatch:{address:req.body.email}}})
			.exec()
			.then(user =>{
				if(user.length > 0){
								...
								.then(countuser=>{
									...
									if(emailOTP){
										...
										user.save()
											.then(result =>{
												if(result){
													request({
											            ...
											            .then(source=>{
											            	res.status(201).json({message:"USER_CREATED",ID:result._id})
											            })
											    		.catch(err =>{
															res.status(500).json({
											...
											.catch(err =>{
												res.status(500).json({
													error: "There is something wrong while inserting data"
												});
											});
									}		
								})
